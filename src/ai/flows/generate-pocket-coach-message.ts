...
const generatePocketCoachMessageFlow = ai.defineFlow(
    {
        name: 'generatePocketCoachMessage',
        inputSchema: PocketCoachInputSchema,
        outputSchema: PocketCoachOutputSchema,
    },
    async (input) => {
        const prompt = `You are a wise and empathetic mentor for teachers, like a friendly coach in their pocket.
        A teacher is feeling stressed about: "${input.stressor}".

        Provide one short, encouraging, and actionable piece of advice in the following language: ${input.language}.
